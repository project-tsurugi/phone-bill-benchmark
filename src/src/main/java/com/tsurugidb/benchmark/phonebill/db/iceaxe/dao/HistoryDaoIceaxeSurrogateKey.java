...
public class HistoryDaoIceaxeSurrogateKey implements HistoryDao {
	...
	private TsurugiSqlPreparedStatement<History> createUpdatePs() {
		String sql = "update history"
				...
				+ "payment_categorty = :payment_categorty, start_time = :start_time, time_secs = :time_secs, charge = :charge, df = :df"
				+ " where  sid = :sid";
		return utils.createPreparedStatement(sql, PARAMETER_MAPPING);
	}
	...
	public List<History> getHistories(Key key) {
		...
		String sql2 = "select sid, caller_phone_number, recipient_phone_number, payment_categorty, start_time,"
...
//				+ " and df = 0";
		...
		String sql1 = "select sid, caller_phone_number, recipient_phone_number, payment_categorty, start_time, time_secs,"
				+ " charge, df" + " from history "
				+ "where start_time >= :start and start_time < :end"
				+ " and recipient_phone_number = :recipient_phone_number and payment_categorty = 'R' and df = 0";
		String sql2 = "select sid, caller_phone_number, recipient_phone_number, payment_categorty, start_time, time_secs,"
				+ " charge, df" + " from history "
				+ "where start_time >= :start and start_time < :end"
