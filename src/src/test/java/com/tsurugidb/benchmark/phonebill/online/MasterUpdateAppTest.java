...
class MasterUpdateAppTest extends AbstractJdbcTestCase {
	...
	void testExec() throws IOException, Exception {
		...
		assertEquals(DateUtils.toDate("2010-11-11"), expected.get(80).startDate);
		assertEquals(DateUtils.toDate("2020-08-08"), expected.get(81).startDate);
		assertEquals(DateUtils.toDate("2017-11-12"), expected.get(80).endDate);
	...
	private void setEndDate(Contract c, int days) {
		c.endDate = new Date(c.startDate.getTime() + days * DateUtils.A_DAY_IN_MILLISECONDS);
	}
	...
	void testCommonDuration() {
		...
		///////////////////////////////////////////////////// contractsの要素数が1のとき
		c1.startDate = DateUtils.toDate("2018-01-01");
		c1.endDate = DateUtils.toDate("2018-05-01");

		// C1の期間にC2の期間が含まれるケース
		c2.startDate = DateUtils.toDate("2018-03-10");
		c2.endDate = DateUtils.toDate("2018-04-01");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));


		// C2の期間にC1の期間が含まれるケース
		c2.startDate = DateUtils.toDate("2017-01-10");
		c2.endDate = DateUtils.toDate("2019-05-01");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));


		// c1 < c2 のケース
		c2.startDate = DateUtils.toDate("2019-01-10");
		c2.endDate = DateUtils.toDate("2019-05-01");
		assertFalse(MasterUpdateApp.commonDuration(c1, contracts));

		// c1の終了月とc2の開始月が連続する月になるケース(c1の開始日の方がc2の開始日より早い)
		c2.startDate = DateUtils.toDate("2018-06-01");
		c2.endDate = DateUtils.toDate("2019-05-01");
		assertFalse(MasterUpdateApp.commonDuration(c1, contracts));


		// c1の終了月とc2の開始月が一致するケース
		c2.startDate = DateUtils.toDate("2018-05-01");
		c2.endDate = DateUtils.toDate("2019-05-01");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));

		c2.startDate = DateUtils.toDate("2018-05-31");
		c2.endDate = DateUtils.toDate("2019-05-01");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));

		// c1の一部の期間とc2の一部の期間がかぶるとき(c1の開始日の方がc2の開始日より早い)
		c2.startDate = DateUtils.toDate("2018-03-01");
		c2.endDate = DateUtils.toDate("2019-05-01");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));

		// c1の一部の期間とc2の一部の期間がかぶるとき(c1の開始日の方がc2の開始日より遅い)
		c2.startDate = DateUtils.toDate("2017-06-01");
		c2.endDate = DateUtils.toDate("2018-02-31");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));

		// c1の開始月とc2の終了月が一致するケース
		c2.startDate = DateUtils.toDate("2017-06-01");
		c2.endDate = DateUtils.toDate("2018-01-31");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));

		c2.startDate = DateUtils.toDate("2017-06-01");
		c2.endDate = DateUtils.toDate("2018-01-01");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));

		// c1の終了月とc2の開始月が連続する月になるケース(c1の開始日の方がc2の開始日より遅い)
		c2.startDate = DateUtils.toDate("2017-06-01");
		c2.endDate = DateUtils.toDate("2017-12-31");
		assertFalse(MasterUpdateApp.commonDuration(c1, contracts));

		// c2 < c1 のケース
		c2.startDate = DateUtils.toDate("2015-01-10");
		c2.endDate = DateUtils.toDate("2015-05-01");
		assertFalse(MasterUpdateApp.commonDuration(c1, contracts));
		...
		c1.startDate = DateUtils.toDate("2013-01-01");
		c1.endDate = null;
		c2.startDate = DateUtils.toDate("2015-01-10");
		c2.endDate = DateUtils.toDate("2015-05-01");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));
		...
		c2.startDate = DateUtils.toDate("2011-01-10");
		c2.endDate = DateUtils.toDate("2011-05-01");
		assertFalse(MasterUpdateApp.commonDuration(c1, contracts));
		...
		// c1,c2のend_dateがともにnullで、c1.start_date < c2_start_dateのケース
		c1.startDate = DateUtils.toDate("2015-01-01");
		c1.endDate = null;
		c2.startDate = DateUtils.toDate("2013-01-10");
		c2.endDate = null;
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));
		...
		c1.startDate = DateUtils.toDate("2018-01-01");
		c1.endDate = DateUtils.toDate("2018-05-01");
		c2.startDate = DateUtils.toDate("2019-03-10");
		c2.endDate = DateUtils.toDate("2019-04-01");
		c3.startDate = DateUtils.toDate("2020-03-10");
		c3.endDate = DateUtils.toDate("2020-04-01");
		c4.startDate = DateUtils.toDate("2021-03-10");
		c4.endDate = DateUtils.toDate("2021-04-01");
		assertFalse(MasterUpdateApp.commonDuration(c1, contracts));


		// 期間が重複する契約が一つだけあるケース
		c1.startDate = DateUtils.toDate("2019-01-01");
		c1.endDate = DateUtils.toDate("2019-12-01");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));


		// すべての契約の期間が重複するケース
		c1.startDate = DateUtils.toDate("2000-01-01");
		c1.endDate = DateUtils.toDate("2119-12-01");
		assertTrue(MasterUpdateApp.commonDuration(c1, contracts));
	}
	...
	void testUpdater1() throws SQLException, IOException {
		...
		contract.endDate = DateUtils.toDate("2020-02-22");
	...
	void testUpdater2() throws IOException, SQLException {
		...
		for (int i = 0; i < 100; i++) {
			...
			updater.update(contract);
			assertEquals(DateUtils.toDate("2020-02-15"), contract.endDate);
		}
		...
		for (int i = 0; i < 100; i++) {
			Contract contract = new Contract();
			contract.startDate = DateUtils.toDate("2020-02-13");
			contract.endDate = null;
			updater.update(contract);
